...
import { BehaviorSubject, Observable, auditTime, merge, startWith, takeUntil, tap, throttle, throttleTime } from 'rxjs';
...
export class SelectBoxComponent implements OnInit {
  ...
  @Input() mode = SelectBoxMode.Hover;
  @Input() selectedElement!: RxELementModel | null;
  public hoverRxElement!: RxELementModel | null;
  public Mode = SelectBoxMode;
  public show = false;
  ...
  initListener() {
    if (this.mode === this.Mode.Hover) {
      this.initHoverElementListener();
      this.initLeaveElementListener();
    }
    if (this.mode === this.Mode.Selected) {
      this.initSelectElementListener();
      this.initCancelSelectElementListener();
    }
  }
  ...
  initStyle() {
    ...
    switch (true) {
      case this.mode === SelectBoxMode.Selected:
        style.border = '2px solid rgb(34, 183, 242)';
        break;
      case this.mode === SelectBoxMode.Hover:
  ...
  initHoverElementListener() {
    elementHooks.hoverSelectElement.subscribe((selectElement) => {
      this.show = true;
      this.hoverRxElement = selectElement;
      this.setPosition(selectElement.hostEl.getBoundingClientRect());
    });
  }

  initLeaveElementListener() {
    elementHooks.leaveSelectElement.subscribe((selectElement) => {
      this.show = false;
    });
  }

  initSelectElementListener() {
    const selectedElement = this.selectedElement;
    if(!selectedElement) return;
    this.show = true;
    const handleSelectElement = (element: RxELementModel) => {
      merge(
        selectedElement.hooks.positionChange,
        selectedElement.hooks.sizeChange
      ).pipe(
        takeUntil(elementHooks.cancelSelectElement)
      ).subscribe((rectInfo) => {
        this.setPosition(rectInfo);
        this.cd.detectChanges()
      });
    }
    selectedElement.hooks.onSelect.pipe(
    ).subscribe(element => handleSelectElement(element))
  }

  initCancelSelectElementListener() {
    const selectedElement = this.selectedElement;
    if(!selectedElement) return;
    // 取消监听元素位置变化
  ...
  deleteElement() {
    ...
    this.editingElementsService.deleteElements(this.selectedElement.uid);
  }
}
